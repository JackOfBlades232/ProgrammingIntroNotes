...
static void show_message(int x, int y)
{
    move(y, x);
    addstr(message);
    mvprintw(0, 0, "(%d, %d)    ", x, y); /* also printw */
    refresh();
}
...
int main()
{
    ...
    initscr();
    cbreak(); /* use CTRL-C and CTRL-D (also -- raw(), to ignore them */
    keypad(stdscr, 1); /* enable escape sequences for arrows and others */
    noecho(); /* disable display of user input */
    curs_set(0);
    getmaxyx(stdscr, row, col);
