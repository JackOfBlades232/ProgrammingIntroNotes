...
{
    int n = bg*8 + fg + 1;
    init_pair(n, fg, bg); /* sets a color pair with number n to a fg-bg */
    return n;
}
...
static void make_line(int line, int width, int fgcolor)
{
    ...
    for (i = 0; i < color_count; i++) {
        ...
        for (j = 0; j < w; j++) {
            att = COLOR_PAIR(pn);
            if (j % 2 == 0)
                att |= A_BOLD; /* spec attributes are bits in int attr */
            if ((j / 2) % 2 == 1)
                att |= A_BLINK;

            attrset(att);
            addch('*'); /* can also do '*' | A_BOLD | A_BLINK */
            refresh();
        }
    }
}
...
static void shift_pairs(int shift)
{
    ...
    for (i = 1; i <= color_count * color_count; i++) {
        ...
        init_pair(i, fg, bg); /* chaging pair colors changes color on scr */
    }
}
...
int main()
{
    ...
    if (!has_colors()) {
        endwin();
        fprintf(stderr, "Can't show colors on a BW srceen\n");
        return 1;
    }
    cbreak();
    keypad(stdscr, 1);
