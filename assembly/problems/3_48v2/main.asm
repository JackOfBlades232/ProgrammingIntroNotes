...
%macro read_cli_uns 2           ; params: offset from esp (int/reg), err label
        push dword 0
        push dword [esp+%1+4]
        call read_uns
        add esp, 8
...
%macro write_float_elem 6       ; params: dgts adr, dgts ln, dgts after .,
        push dword %1           ;   pref adr, pref ln, fd
        push dword %2
        push dword %3
        call write_float
        add esp, 12
        ...
        call_write_str %4, %5, %6
        call_write_str %1, %2-1, %6 ; discard #0
%endmacro
...
section .text
        ; preparations
_start: cmp dword [esp], 6          ; must be cli args: file name,
        ...
        ; read max digits after . for output
        read_cli_uns 24, .err
        mov [dg_lim], eax
...
.again: write_horiz_line [line_ch], line_ln, [fd]
        ...
        write_float_elem dgts, dgts_ln, [dg_lim], pref, pref_ln, [fd]
        
        mov ebx, 4                  ; now, empty stack to file (5 numbers)
.line_lp:
        write_float_elem dgts, dgts_ln, [dg_lim], sep, sep_ln, [fd]
        dec ebx
        cmp ebx, 0
        jnz .line_lp
